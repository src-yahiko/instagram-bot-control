...
# Generating a Email
def generatingTrashmail():
    url = 'https://email-fake.com/'
    req = requests.get(url)
    soup = BeautifulSoup(req.content, "html.parser")
    mail = soup.find_all("span", {"id": "email_ch_text"})
    return mail[0].contents[0] # Returns Email as String

def generate():
    return({
        "fullname" : generatingName(),
        "username" : username(size=12, chars=string.ascii_lowercase + random.choice(['.', '_'])),
        "password" : generatePassword(),
        "trashmail" : generatingTrashmail()
    })

if __name__=='__main__':
    print(json.dumps(generate(), indent=4, sort_keys=True))